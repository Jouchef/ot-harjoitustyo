...
class UserDashboard:
    def __init__(self, root):
        ...
        Radiobutton(role_frame, text="Ryhmänjohtaja",
                    variable=self.user_role_var, value="Ryhmänjohtaja").pack(side="left")
        Radiobutton(role_frame, text="Admin",
                    variable=self.user_role_var, value="Admin").pack(side="left")

        ttk.Label(self.frame, text="Group:").pack(anchor="w")
        self.user_group_combobox = ttk.Combobox(
            self.frame, textvariable=self.user_group_var)
        self.user_group_combobox.pack()
        self.update_group_combobox()  # Täytetään valikko alussa

        ttk.Button(self.frame, text="Add User",
                   command=self.create_user).pack()
        ...
        ttk.Button(self.frame, text="Add Group",
                   command=self.create_group).pack()
        ...
        ttk.Button(self.frame, text="Add to Group",
                   command=self.add_user_to_group).pack()
        ttk.Button(self.frame, text="Remove from Group",
                   command=self.remove_user_from_group).pack()
    ...
    def add_user_to_group(self):
        ...
        user = next((u for u in self.user_service.list_users()
                    if u.username == username), None)
        group = next((g for g in self.group_service.list_groups()
    ...
    def update_user_list(self):
        ...
        for user in self.user_service.list_users():
            self.user_listbox.insert(
                'end', f"{user.username} ({user.role}) - Group: {user.group}")
    ...
    def update_group_list(self):
        ...
        for group in self.group_service.list_groups():
            ...
            self.group_listbox.insert(
                'end', f"{group.name} - ({group.calendars}) - Participants: {participants}")

    def update_group_combobox(self):
      # Päivittää Comboboxin vaihtoehdot nykyisten ryhmien nimillä
        group_names = [
            group.name for group in self.group_service.list_groups()]
